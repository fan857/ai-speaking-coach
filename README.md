...
- 前端接入浏览器真实录音采集能力。
- “录音占位”按钮改为“开始录音 / 停止录音”。
- 使用浏览器原生 `MediaRecorder` 生成录音音频。
- 页面展示录音时长、音频大小和音频回放控件。
- 当前 PR 仍不做真实语音识别，文本框继续用于模拟识别结果。

## 技术栈

- 前端：React, Vite
- 后端：Node.js, Express
- 当前阶段：不接入真实 OpenAI API，不接入真实语音识别或 TTS；已支持浏览器录音采集，仍使用 mock 数据模拟 AI 反馈。
...
4. 在文本框输入一句英文，模拟语音识别结果。
5. 点击“提交模拟语音”。
6. 前端请求后端 mock API。
7. 页面会展示 AI 回复、纠错建议和四项评分。
