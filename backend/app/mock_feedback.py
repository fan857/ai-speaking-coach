...
def get_mock_pronunciation_result(scenario_id: str, reference_text: str, transcript: str) -> dict[str, Any]:
    """Mock result for pronunciation imitation assessment."""
    from app.pronunciation import align_words, compute_accuracy_score, get_difficult_words
    ...
    if accuracy >= 90:
        tips = ["????????????", "??????????????????"]
        phoneme_notes = ["?? /th/ ??? the?that????????"]
        encouragement = "??????????????????"
    elif accuracy >= 70:
        tips = ["????????????????", f"????????{", ".join(difficult)}"]
        phoneme_notes = ["?? /r/ ? /l/ ????? right vs light?"]
        encouragement = "?????????????????????"
    else:
        tips = ["???????????????????", "????????????????"]
        phoneme_notes = ["???????? ship vs sheep???????????"]
        encouragement = "??????????????????"
